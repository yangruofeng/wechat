...
function(a) {
    ...
    n.popBottom = function(a) {
        ...
        n.popBottom.index = n.open(i.extend({
            type: 1,
            content: a.content || "",
            shade: !1,
    ...
    u.prototype.on = function(a, e) {
        return "function" == typeof e && (o[a] ? o[a].push(e) : o[a] = [e]),
        this
    },
    u.prototype.chat = function(a) {
        if (window.JSON && window.JSON.parse) return b(a),
        this
    },
    ...
    var r = function(a) {
        ...
        ["{{# var length = 0; layui.each(" + a.item + ", function(i, data){ length++; }}", '<li layim-event="chat" data-type="' + a.type + '" data-index="{{ ' + (a.index || "i") + ' }}" id="layim-' + a.type + '{{ data.id }}" {{ data.status === "offline" ? "class=layim-list-gray" : "" }}><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>', "{{# }); if(length === 0){ }}", '<li class="layim-null">' + (e[a.type] || "暂无数据") + "</li>", "{{# } }}"].join("")
    },
    d = ['<div class="layui-layim">', '<div class="layim-chat-title" style="background-color: {{d.base.chatTitleColor}};">',"{{# if(!d.base.findFriendList && !d.base.notification){ }}",'<i class="layui-icon layim-chat-back" layim-event="backList" id="backKhbuy">&#xe603;</i>',"{{# } else {  }}",'<i class="layui-icon layim-chat-back" id="backFriendList">&#xe603;</i>','<a href="javascript:;" style="display: none;" id="search_user_list" class="message-i-right-opera">Search</a>',"{{# } }}", "<p>{{ d.base.title }}</p>","{{# if(d.base.add){ }}",'<a class="add-friend" id="add_friend">+</a>', "{{# } }}","{{# if(d.base.clearHistory){ }}",'<a class="clearhistory" id="btn-clear-history"><i class="icon-trash" style="color: #FFFFFF"></i></a>', "{{# } }}", "</div>",'<div class="msg-notification" id="msgNotification"></div>', '<ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">', '{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}', "<li>", '<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>', '<ul class="layui-layim-list {{# if(spread === "true"){ }}', " layui-show", '{{# } }}">', r({
        type: "friend",
        item: "item.list",
        index: "index"
    }), "</ul>", "</li>", "{{# }); if(d.friend.length === 0){ }}", '<li>', "{{# } }}", "</ul>", '<ul class="layui-unselect layui-layim-tab">', '<li class="layui-icon" title="会话" layim-event="tab" lay-type="history">&#xe611;</li>', '<li class="layui-icon" title="联系人" layim-event="tab" lay-type="friend">&#xe612;</li>', "</ul>", "</div>"].join(""),
    m = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">', "{{# if(d.base.chatTitleColor){ }}", '<div class="layim-chat-title" style="background-color: {{d.base.chatTitleColor}};">', '<p><i class="layui-icon layim-chat-back" id="backFriendList">&#xe603;</i> {{ d.data.name }}<span class="layim-chat-status"></span> </p>',"{{# if(d.base.clearHistory){ }}", '<a class="clearhistory" id="btn-clear-history"><i class="icon-trash" style="color: #FFFFFF"></i></a>',"{{# } }}", "</div>", "{{# } }}", '<div class="layim-chat-main" {{d.base.chatTitleColor ? "" : "nonetitle"}}>', "<ul></ul>", "</div>", '<div class="layim-chat-footer">', '<div class="layim-chat-send"><input type="text" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">发送</button></div>', '<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">', '<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face">&#xe60c;</span>',  '<span class="layui-icon layim-tool-image" title="上传图片" >&#xe60d;<input type="file" name="file" accept="image/*" id="sendImages" class="sendImages"></span>', "{{# if(d.base && d.base.uploadFile){ }}", '<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>', "{{# }; }}", "{{# layui.each(d.base.tool, function(index, item){ }}", '<span class="layui-icon layim-tool-{{item.alias}}" title="{{item.title}}" layim-event="extend" lay-filter="{{ item.alias }}">{{item.icon}}</span>', "{{# }); }}", "</div>", "</div>", "</div>"].join(""),
    y = function(a) {
        return a < 10 ? "0" + (0 | a) : a
    };
    layui.data.date = function(a) {
        var e = new Date(a || new Date);
        return y(e.getMonth() + 1) + "-" + y(e.getDate()) + " " + y(e.getHours()) + ":" + y(e.getMinutes())
    },
    layui.data.content = function(a) {
        ...
        .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g,function(a) {
            ...
            i = (a.match(/\)\[([\s\S]*?)\]/) || [])[1];
            return e ? '<a class="layui-layim-file" href="' + e + '" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (i || e) + "</cite></a>": a
        })
    ...
    var p, f = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">', '<div class="layim-chat-user"><img src="{{ d.avatar }}">', "</div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp;") }}</div>', "</li>"].join(""),
    h = {
        message: {},
        chat: []
    },
    ...
    b = function(a) {
        a = a || {};
        var e = (i("#layui-layim-chat"), {
            data: a,
            base: h.base,
            local: h.local
        });
        return a.id ? (n.close(b.index), b.index = n.open({
            type: 1,
    ...
    k = function() {
        var a = {
            ...
            avatar: h.mine ? h.mine.avatar: layui.cache.dir + "css/pc/layim/skin/logo.jpg",
        ...
        if (a.content = e.textarea.val(), "" !== a.content) {
            ...
            c - (k.time || 0) > 6e4 && (i.append('<li class="layim-chat-system"><span>' + layui.data.date() + "</span></li>"), k.time = c),
    ...
    S = function(a) {
        ...
        var e = {};
        console.log(a)
        if (a.timestamp = a.timestamp || (new Date).getTime(), a.system || j(a), C = JSON.parse(JSON.stringify(a)), h.base.voice && w(), !p && a.content) {
            if (h.message[a.type + a.id]) h.message[a.type + a.id].push(a);
            else if (h.message[a.type + a.id] = [a], "friend" === a.type) {
                var i;
                layui.each(h.friend,
                function(e, t) {
                    if (layui.each(t.list,
                    function(e, t) {
                        if (t.id == a.id) return t.type = "friend",
                        t.name = t.username,
                        h.chat.push(t),
                        i = !0
                    }), i) return ! 0
                ...
                layui.each(h.group,
                function(e, i) {
                    if (i.id == a.id) return i.type = "group",
                    i.name = i.groupname,
                    h.chat.push(i),
                    n = !0
                }),
            ...
            h.chat.push(a);
            "group" === a.type && layui.each(h.group,
            function(i, t) {
                if (t.id == a.id) return e.avatar = t.avatar,
                !0
            }),
            !a.system
        }
        if (p) {
            ...
            a.system ? s.append('<li class="layim-chat-system"><span>' + a.content + "</span></li>") : "" !== a.content.replace(/\s/g, "") && (a.timestamp - (k.time || 0) > 6e4 && (s.append('<li class="layim-chat-system"><span>' + layui.data.date(a.timestamp) + "</span></li>"), k.time = a.timestamp), s.append(t(f).render(a))),
            T()
        }
    },
    j = function(a) {
        ...
        i = e.chatlog || {};
        i[a.type + a.id] ? (i[a.type + a.id].push(a), i[a.type + a.id].length > c && i[a.type + a.id].shift()) : i[a.type + a.id] = [a],
        e.chatlog = i,
    ...
    N = function() {
        ...
        layui.each(i[e.data.type + e.data.id],
        function(a, e) { (new Date).getTime() > e.timestamp && e.timestamp - (k.time || 0) > 6e4 && (n.append('<li class="layim-chat-system"><span>' + layui.data.date(e.timestamp) + "</span></li>"), k.time = e.timestamp),
    ...
    T = function() {
        ...
        if (t.length >= c) {
            ...
            i.prev().hasClass("layim-chat-system") || i.before(''),
            n.remove()
        }
    ...
    D = function() {
    	...
    	var a = ["[):]","[:D]","[;)]","[:-o]","[:p]","[(H)]","[:@]","[:s]","[:$]","[:(]","[:\'(]","[:|]","[(a)]","[8o|]","[8-|]","[+o(]","[<o)]","[|-)]","[*-)]","[:-#]","[:-*]","[^o)]","[8-)]","[(|)]","[(u)]","[(S)]","[(*)]","[(#)]","[(R)]","[({)]","[(})]","[(k)]","[(F)]","[(W)]","[(D)]"];
    	e = {};
        return layui.each(a,function(index, i) {
        	index = index + 1;
            e[i] = layui.cache.dir + 'images/face/ee_'+ index + '.png';
        }),
        e
    } (),
    O = layui.stope,
    J = function(a, e, i) {
        var t, n = a.value;
        i || a.focus(),
        document.selection ? (t = document.selection.createRange(), document.selection.empty(), t.text = e) : (t = [n.substring(0, a.selectionStart), e, n.substr(a.selectionEnd)], i || a.focus(), a.value = t.join(""))
    },
    B = {
        chat: function(a) {
            var e = layui.data("layim-mobile")[h.mine.id] || {},
            ...
            l = {};
            "friend" === i ? l = h[i][t].list[n] : "group" === i ? l = h[i][n] : "history" === i && (l = (e.history || {})[t] || {}),
        ...
        image: function(a) {
            ...
            l({
                ...
                success: function(a) {
                    ...
                    "images" === e ? J(t.textarea[0], "img[" + (a.data.src || "") + "]") : "file" === e && J(t.textarea[0], "file(" + (a.data.src || "") + ")[" + (a.data.name || "下载文件") + "]"), k()) : n.msg(a.msg || "上传失败")
                }
            })
        },
        extend: function(a) {
