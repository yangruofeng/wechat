...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto','upload'], function(exports){
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<a href=" http://localhost/shopnc/wap/index.php?act=message&op=conversation" class="header-back" style="top:3px;left:0;"><span style="margin-left:0;padding-left:0;"><i class="icon-angle-left"></i></span></a>'
    ,'<p>{{ d.data.name }}<span class="layim-chat-status"></span></p>'
    ,'<a href="javascript:;" id="btn-clear-history" class="i-main-opera" style="top:5px;right:10px;"><span class="clear_chat_record" style="background: none;font-size: 25px"><i class="icon-trash" style="color: #FFFFFF"></i></span></a>'
    ,'</div>'
    ,'{{# } }}'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-send"><input type="text" autocomplete="off" id="talkInputId"><button class="layim-send layui-disabled" layim-event="send">'+WEBIM_SEND+'</button></div>'
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ,'<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face"><img style="width:28px;" src="'+layui.cache.dir +'/images/emoji.png" /></span>'
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image"><img style="width:26px;margin-top:2px;" src="'+layui.cache.dir +'/images/upload_img.png" /><input type="file" accept="image/*" class="im_chat_upload sendImages"  id="sendImages"></span>'
  ...
  var sendMessage = function(){
    ...
    var send_con = data.content;
    if(send_con.indexOf('img[') > -1){
    	sendPic();
    }else{
    	sendText();
    }
