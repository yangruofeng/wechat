...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    var nodata = {
      friend: WEBIM_PACKET_NO_FRIENDS
      ,group: WEBIM_NO_GROUP
      ,history: WEBIM_NO_SESSION
    };
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}" {{ data.status === "offline" ? "class=layim-list-gray" : "" }}><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"'+WEBIM_NAMELESS+'" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>'
    ,'{{# }); if(length === 0){ }}'
      ,'<li class="layim-null">'+ (nodata[options.type] || WEBIM_NO_DATA) +'</li>'
    ,'{{# } }}'].join('');
  };
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}'
      ,' layim-tab-two {{# } }} {{# if(!d.base.isconversation && !d.base.isgroup){  }} layim-tab-one'  
    ,'{{# } }}">'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isfriend){ }}'
      ,' layim-hide'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="'+WEBIM_CONTACTS+'" layim-event="tab" lay-type="friend">&#xe612;</li>'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isgroup){ }}'
      ,' layim-hide'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="'+WEBIM_GROUP+'" layim-event="tab" lay-type="group">&#xe613;</li>'
      ,'<li class="layui-icon '
      ,'{{# if(!d.base.isconversation){ }}'
      ,' layim-hide'
      ,'{{# } else if(!d.base.isfriend) { }}'
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="'+WEBIM_HISTORICAL_CONVERSATION+'" layim-event="tab" lay-type="history">&#xe611;</li>'
    ,'</ul>'
    ,'<ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">'
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"'+WEBIM_UNNAMED_GROUPING+'"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
    ...
    ,'{{# }); if(d.friend.length === 0){ }}'
      ,'<li><ul class="layui-layim-list layui-show"><li class="layim-null">'+WEBIM_NO_CONTACT+'</li></ul>'
    ,'{{# } }}'
    ,'</ul>'
    ,'<ul class="11 layui-unselect layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup && !d.base.isconversation ){ }}layui-show{{# } }}">'
      ,'<li>'
      ...
      ,'</li>'
    ,'</ul>'
    ,'<ul class="22 layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup && d.base.isconversation){ }}layui-show{{# } }}">'
      ,'<li>'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ,'<li class="layui-icon layim-tool-search" layim-event="search" title="'+WEBIM_SEARCH+'">&#xe615;</li>'
      ,'{{# if(d.base.msgbox){ }}'
      ,'<li class="layui-icon layim-tool-msgbox" layim-event="msgbox" title="'+WEBIM_MESSAGE_BOX+'">&#xe645;<span class="layui-anim"></span></li>'
      ,'{{# } }}'
      ,'{{# if(d.base.find){ }}'
      ,'<li class="layui-icon layim-tool-find" layim-event="find" title="'+WEBIM_FIND+'">&#xe608;</li>'
      ,'{{# } }}'
      ,'{{# if(!d.base.background){ }}'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img src="{{ d.data.avatar }}"><span class="layim-chat-username" id="{{ d.data.id }}" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"'+WEBIM_NAMELESS+'" }} {{d.data.temporary ? "<cite>'+WEBIM_TEMPORARY_SESSION+'</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ,'<span class="layui-icon layim-tool-face" title="'+WEBIM_SELECT_EXPRESSION+'" layim-event="face">&#xe60c;</span>'
        //,'{{# if(d.base && d.base.uploadImage){ }}'
        ,'<span class="layui-icon layim-tool-image" title="'+WEBIM_SEND_PICTURE+'" layim-event="image">&#xe60d;<input type="file" id="sendImages{{ d.data.id }}" class="sendImages" accept="image/*"></span>'
        //,'{{# }; }}'
        ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="'+WEBIM_SEND_FILE+'" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'
        ...
        ,'<span class="layim-tool-log" layim-event="chatLog"><i class="layui-icon">&#xe60e;</i>'+WEBIM_CHAT_LOG+'</span>'
        ,'{{# }; }}'
      ,'</div>'
      ...
      ,'<div class="layim-chat-bottom">'
        ,'<div class="layim-chat-send">'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="layim-send-close" layim-event="closeThisChat">'+DIALOG_SHUT_DOWN+'</span>'
          ,'{{# } }}'
          ,'<span class="layim-send-btn" layim-event="send">'+WEBIM_SEND+'</span>'
          ,'<span class="layim-send-set" layim-event="setSend" lay-type="show"><em class="layui-edge"></em></span>'
          ,'<ul class="layui-anim layim-menu-box">'
            ,'<li {{d.local.sendHotKey !== "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend" lay-type="Enter"><i class="layui-icon">&#xe618;</i>'+WEBIM_ENTER_SEND_MESSAGE+'</li>'
            ,'<li {{d.local.sendHotKey === "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend"  lay-type="Ctrl+Enter"><i class="layui-icon">&#xe618;</i>'+WEBIM_CTRL_ENTER_SEND_MESSAGE+'</li>'
          ,'</ul>'
        ,'</div>'
      ,'</div>'
    ,'</div>'
  ...
  var elemAddTpl = ['<div class="layim-add-box">'
    ...
    ,'{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}'
      ,'<p>'+WEBIM_SELECT_GROUP+'</p>'
    ,'{{# } if(d.data.type === "friend"){ }}'
    ,'<select class="layui-select" id="LAY_layimGroup">'
      ,'{{# layui.each(d.data.group, function(index, item){ }}'
      ,'{{# if(item.id != 1){ }}'
      //,'<option value="{{ item.id }}">{{ item.groupname }}</option>'
      ,'<option value="2">'+WEBIM_FRIEND_LIST+'</option>'
      ,'{{# } }}'
      ,'{{# }); }}'
    ,'</select>'
    ,'{{# } }}'
    ,'{{# if(d.data.type === "group"){ }}'
      ,'<p>'+WEBIM_ENTER_VALIDATION_INFORMATION+'</p>'
    ,'{{# } if(d.type !== "setGroup"){ }}'
      ,'<textarea id="LAY_layimRemark" placeholder="'+WEBIM_VALIDATION_INFORMATION+'" class="layui-textarea"></textarea>'
    ,'{{# } }}'
    ,'</div>'
  ,'</div>'].join('');
  ...
  layui.data.content = function(content){
    ...
    .replace(/a\[([^\s]+?)\]/g, function(a){  //转义链接
    	var text = a.replace(/(^a\[)|(\]$)/g, '');
      return '<a href="'+text+'" style="text-decoration: underline;">'+text+'</a>';
    })
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>'
    ...
    ,'{{# if(d.mine){ }}'
      ,'<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"'+WEBIM_NAMELESS+'" }}'
     ,'{{# } else { }}'
      ,'{{ d.username||"'+WEBIM_NAMELESS+'" }}<i>{{ layui.data.date(d.timestamp) }}</i>'
     ,'{{# } }}'
      ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>'
  ,'</li>'].join('');
  
  var elemChatList = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"'+WEBIM_NAMELESS+'" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-history').on('contextmenu', 'li', function(e){
      var othis = $(this);
      var html = '<ul data-id="'+ othis[0].id +'" data-index="'+ othis.data('index') +'"><li layim-event="menuHistory" data-type="one">'+WEBIM_REMOVE_SESSION+'</li><li layim-event="menuHistory" data-type="all">'+WEBIM_CLEAR_ALL_SESSION_LISTS+'</li></ul>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title|| WEBIM_MY_LAYIM) +'</span>'
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(!data.id){
      return layer.msg(WEBIM_ILLEGAL_USER);
    }
  ...
  var popAdd = function(data, type){
    ...
    return popAdd.index = layer.open({
      ...
      ,area: '430px'
      ,title: {
        friend: WEBIM_ADD_FRIEND
        ,group: WEBIM_JOIN_GROUP
      }[data.type] || ''
      ,shade: false
      ,resize: false
      ,btn: type ? [DIALOG_DETERMINE, DIALOG_CANCEL] : [WEBIM_SEND_APPLICATION, DIALOG_SHUT_DOWN]
  ...
  var sendMessage = function(){
    var data = {
      username: cache.mine ? cache.mine.username : WEBIM_VISITOR
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      ...
      if(data.content.length > maxLength){
        return layer.msg(WEBIM_NEWS_FROM.replace('10', maxLength))
      }
    ...
    if((!layimChat && data.content) || index === -1){
      ...
      if(!data.system){
        if(cache.base.notice){
          notice({
            title: WEBIM_NEWS_FROM.replace('xx', data.username)
            ,content: data.content
            ,avatar: group.avatar || data.avatar
          });
        }
        return setChatMin({
          name: WEBIM_NEW_MESSAGE_RECEIVED
          ,avatar: group.avatar || data.avatar
          ,anim: 6
        });
      }
      ...
      if(data.type === 'friend'){
        layui.each(cache.friend, function(index, item){
          if(data.groupid == item.id){
            ...
            if(has) return layer.msg(WEBIM_FRIEND_ALREADY_EXISTS.replace('xx', data.username),{anim: 6});
          ...
          layui.each(item1.list, function(index, item){
            if(data.id == item.id){
              ...
              if(cache.friend[index1].list.length === 0){
                li.find('.layui-layim-list').html('<li class="layim-null">'+WEBIM_GROUP_HAS_NO_FRIENDS+'</li>');
              }
              return true;
            }
          });
        ...
        layui.each(cache.group, function(index, item){
          if(data.id == item.id){
            ...
            if(cache.group.length === 0){
              listElem.html('<li class="layim-null">'+WEBIM_NO_GROUP+'</li>');
            }
            return true;
          }
        });
  ...
  var chatListMore = function(){
    ...
    if(ul.find('li').length >= MAX_ITEM){
      ...
      if(!ul.prev().hasClass('layim-chat-system')){
        ul.before('<div class="layim-chat-system"><span layim-event="chatLog">'+WEBIM_VIEW_MORE_RECORDS+'</span></div>');
      }
      first.remove();
    }
  ...
  var faces = function(){
	  var arr = {};
	    ...
	    var alt = ["[):]","[:D]","[;)]","[:-o]","[:p]","[(H)]","[:@]","[:s]","[:$]","[:(]","[:\'(]","[:|]","[(a)]","[8o|]","[8-|]","[+o(]","[<o)]","[|-)]","[*-)]","[:-#]","[:-*]","[^o)]","[8-)]","[(|)]","[(u)]","[(S)]","[(*)]","[(#)]","[(R)]","[({)]","[(})]","[(k)]","[(F)]","[(W)]","[(D)]"];
		layui.each(alt, function(index, item){
			index = index + 1;
			arr[item] = layui.cache.dir + 'images/face/ee_'+ index + '.png';
	    });
	    return arr;
  }();
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,search: function(othis){
      ...
      var input = search.find('input'), find = function(e){
        ...
        if(val === ''){
          ...
          var data = [], friend = cache.friend || [],username = '';
          var group = cache.group || [], html = '';
          for(var i = 0; i < friend.length; i++){
            for(var k = 0; k < (friend[i].list||[]).length; k++){
            	username = friend[i].list[k].username.toLowerCase();
            	val = val.toLowerCase();
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
            	console.log(data[l].username)
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || WEBIM_NAMELESS) +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
            html = '<li class="layim-null">'+WEBIM_NO_SEARCH_RESULTS+'</li>';
          }
          main.html(html);
          events.tab(3);
        }
        ...
        events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
          ...
          ,success: function(layero){
            ...
            post(members, function(res){
              ...
              //获取群员
              othis.find('.layim-chat-members').html(res.members||(res.list||[]).length + WEBIM_PERSON);
        ...
        ,success: function(layero){
          layero.find('.layim-face-list>li').on('mousedown', function(e){
            stope(e);
        	  ...
        	  //var title = 'face' + this.title;
        	  var title = this.title;
        	  if(faces[this.title]){
        		  //title =  faces[this.title] ;
        	  }
            focusInsert(thatChat.textarea[0], title + ' ');
        ...
        ,success: function(res){
          if(res.code == 0){
            ...
            if(type === 'images'){
              ...
              focusInsert(thatChat.textarea[0], 'file('+ (res.data.src||'') +')['+ (res.data.name||WEBIM_DOWNLOAD_FILE) +']');
            }
            sendMessage();
          } else {
            layer.msg(res.msg||WEBIM_UPLOAD_FAILED);
          }
        }
      });
    }
    ...
    ,chatLog: function(othis){
      ...
      if(!cache.base.chatLog){
        return layer.msg(WEBIM_NO_MORE_CHATS);
      }
      layer.close(events.chatLog.index);
      return events.chatLog.index = layer.open({
        type: 2
        ,maxmin: true
        ,title: WEBIM_CHAT_WITH.replace('xx', thatChat.data.name)
        ,area: ['450px', '100%']
        ,shade: false
        ,offset: 'rb'
        ,skin: 'layui-box'
    ...
    ,menuHistory: function(othis, e){
      ...
      var none = '<li class="layim-null">'+WEBIM_NO_SESSION+'</li>'
