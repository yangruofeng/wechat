<div>
    ...
    <form class="form-horizontal" id="repayment_form_one">
        ...
        <table class="table">
            ...
            <?php $i=0; foreach ((new currencyEnum())->Dictionary() as $key => $currency) { $i++ ;?>
            <tr>
                <td class="col-sm-4"><span class="pl-25"><?php echo $lang['number_'.$i].' Currency' ?></span></td>
                <td class="col-sm-8">
                    <div class="input-group" style="max-width: 300px">
                        <?php if ($i == 1) { ?>
                            <input class="form-control currency_amount" type="number" name="<?php echo strtolower($currency) ?>_amount" value="" currency="<?php echo $currency;?>" onchange="calcCurrencyAmount(this)">
                        <?php } else { ?>
                            <input class="form-control currency_amount" type="number" name="<?php echo strtolower($currency) ?>_amount" value="" currency="<?php echo $currency;?>">
                        <?php } ?>
                        <span class="input-group-addon" style="min-width: 80px;border-left: 0;height: 30px;border-radius: 0px"><?php echo $key;?></span>
                    </div>
                </td>
            </tr>
            <?php } ?>
            <input type="hidden" id="chk_over" name="chk_over" value="">
