                    ...
                    <div class="content">
                        <form id="frm_item"  method="POST" enctype="multipart/form-data" action="<?php echo getUrl('mortgage', 'submitTransferToTeller', array(), false, ENTRY_COUNTER_SITE_URL);?>">
                            ...
                            <?php if($output['user_position'] == userPositionEnum::CHIEF_TELLER){ ?>
                                <div  class="col-sm-12">
                                    <input type="hidden" name="safe_id" value="<?php echo $output['safe_id']?>">
                                    <input type="hidden" name="safe_branch_id" value="<?php echo $output['safe_branch_id']?>">
                                    <label class="col-sm-3 control-label">Safe Box </label>
                                    <div class="col-sm-8" style="margin-left: 5px">
                                        <label><?php echo $output['safe_code']?></label>
                                    </div>
                                </div>
                            <?php  }?>
...
//        window.history.back(-1);
        window.location.href='<?php echo getUrl('mortgage', 'myStoragePage', array(), false, ENTRY_COUNTER_SITE_URL)?>';
    }
