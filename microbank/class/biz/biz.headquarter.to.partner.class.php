...
class bizHeadquarterToPartnerClass extends bizBaseClass
{
    public function __construct($scene_code)
    {
        ...
        $this->biz_code = bizCodeEnum::HEADQUARTER_TO_PARTNER;
    ...
    public function execute($partner_id,$user_id,$trading_password,$amount,$currency,$remark)
    {
        ...
        $bankObj = new objectPartnerClass($partner_id);
        ...
        $chkBalance = $accountObj->getPassbookCurrencyBalance();
        if( $chkBalance[$currency] < $amount ){
        ...
        // 更新账本
        $rt = (new headquarterToPartnerTradingClass($partner_id,$amount,$currency,$remark,$userObj->user_name))->execute();
        if( !$rt->STS ){
