 ...
 * Time: 18:37
 */
class bizMemberChangePhoneByCounterClass extends bizBaseClass
{
    public function __construct()
    {
        ...
        $this->biz_code = bizCodeEnum::MEMBER_CHANGE_PHONE_BY_COUNTER;
        $this->bizModel = new biz_member_change_phoneModel();
    }
    ...
    public function execute($user_id,$member_id,$member_image,$sms_id,$sms_code,$country_code,$phone_number,$fee,$currency,$payment_way)
    {
        ...
        $valid = isPhoneNumber($contract_phone);
        if( !$valid ){
            return new result(false,"Not phone number:$country_code - $phone_number",null,errorCodesEnum::INVALID_PHONE_NUMBER);
        }
        ...
        // 电话已被使用
        $is_used = memberClass::checkPhoneNumberIsRegistered($country_code,$phone_number);
        if( $is_used ){
            return new result(false,'Phone number is registered.',null,errorCodesEnum::PHONE_USED);
        }

        $old_phone_id = $member->phone_id;
        ...
        $biz->old_phone_id = $old_phone_id;
        $biz->new_phone_id = $contract_phone;
        $biz->fee = $fee;
