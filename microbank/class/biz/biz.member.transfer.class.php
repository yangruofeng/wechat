 ...
 * Time: 17:30
 */
class bizMemberTransferClass extends bizBaseClass
{
    public function __construct($scene_code)
    {
        ...
        $this->bizModel = new biz_member_transferModel();
    }
    ...
    public function checkBizOpen()
    {
        ...
        if( global_settingClass::isForbiddenTransfer() ){
    ...
    protected function checkBranchDayLimit($branch_id,$amount,$currency)
    {
        ...
        // 当日已经进行的金额
        $done_amount = 0;
        $sql = "select * from biz_member_transfer where biz_code='".$this->biz_code."' and branch_id='$branch_id' 
        ...
        if( $limit && $limit['max_per_day'] && ($done_amount+$request_amount) > $limit['max_per_day'] ){
            return new result(false,'Exceed branch day limit.',null,errorCodesEnum::EXCEEDED_PER_DAY_LIMIT);
        }
    ...
    protected function checkMemberLimit($member_id,$amount,$currency)
    {
        ...
        if( $limit ){
            ...
            // 当日已经进行的金额
            $done_amount = 0;
            $sql = "select * from biz_member_transfer where biz_code='".$this->biz_code."' and member_id='$member_id' 
            and DATE_FORMAT(update_time,'%Y-%m-%d')='".date('Y-m-d')."' and state='".bizStateEnum::DONE."' ";
            $rows = $reader->getRows($sql);
            // 换算成设定的美元
            foreach( $rows as $v ){
    ...
    public function checkMemberTradingPassword($biz_id,$password)
    {
        ...
        $chk = $objectMember->checkTradingPassword($password,'Member Transfer');
        if( !$chk->STS ){
            return $chk;
        }
