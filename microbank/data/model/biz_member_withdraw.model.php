 ...
 * Time: 16:05
 */
class biz_member_withdrawModel extends tableModelBase
{
    public function __construct()
    {
        parent::__construct('biz_member_withdraw');
    }
...
FROM biz_member_withdraw a
INNER JOIN client_member c ON c.`uid` = a.`member_id`
LEFT JOIN partner_trx_api b ON b.`ref_biz_type` = 'savings' AND b.`ref_biz_sub_type` = 'member_withdraw_to_partner' AND b.`ref_biz_id` = a.`uid`
LEFT JOIN passbook_trading d ON d.`uid` = a.`passbook_trading_id`
WHERE a.`biz_code` = 'member_withdraw_to_partner' AND a.`state` = $pending_state $search_filter
ORDER BY a.uid
SQL;
