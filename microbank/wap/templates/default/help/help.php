...
<div class="wrap loan-contract-wrap">
    <div class="aui-tab contract-tab" id="tab">
        <div class="aui-tab-item aui-active">
            <?php echo $lang['label_help_qa']; ?>
        </div>
        <div class="aui-tab-item">
            <div></div><?php echo $lang['label_help_system']; ?>
        </div>
    </div>
    <div class="aui-refresh-content">
        <div class="limit-calculation tab-panel" id="tab-1" type="0">
            <div class="aui-content aui-margin-b-15">
                <ul class="aui-list aui-media-list contract-list" id="help-list-1"></ul>
            </div>
        </div>
        <div class="limit-calculation tab-panel" id="tab-2" type="1" style="display: none;">
            <div class="aui-content aui-margin-b-15">
                <ul class="aui-list aui-media-list contract-list" id="help-list-2"></ul>
            </div>
        </div>
    </div>
    <div class="data-loading" style="display: none;">
        <img src="<?php echo WAP_SITE_URL; ?>/resource/image/loading.gif" alt="">
    </div>
...
<script type="text/html" id="tpl_help_item">
    {{ if( it && it.length > 0 ){ }}
        ...
        <div class="aui-media-list-item-inner">
            <div class="aui-list-item-inner">
                <div class="aui-list-item-text">
                    <div class="aui-list-item-title title">{{=it[i]['help_title']}}</div>
...
<script type="text/javascript">
    ...
    function getPageData() {
        $.ajax({
            type: 'POST',
            url: '<?php echo WAP_SITE_URL;?>/index.php?act=help&op=getHelpData',
            data: {category: category, type: type, page_num: page_num, page_size: page_size},
            dataType: 'json',
            success: function (data) {
                if (data.STS) {
                    ...
                    if (list == null) {
                        ...
                        var interText = doT.template($('#tpl_help_item').text());
                        if (page_num == 1) {
                            $('#help-list-' + index).html(interText(list));
                        } else {
                            $('#help-list-' + index).append(interText(list));
                        }
                        if (total_pages <= 1) {
                            loadDataDone('<?php echo $lang['label_no_data'];?>');
                        } else {
    ...
    function getDetail(id) {
        window.location.href = "<?php echo WAP_SITE_URL;?>/index.php?act=help&op=helpDetail&id=" + id;
    }
