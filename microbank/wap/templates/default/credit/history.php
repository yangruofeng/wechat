...
<?php include_once(template('widget/inc_location_header'));?>
<div class="wrap credit-history-wrap">
  <div class="aui-refresh-content">
    <ul class="aui-list aui-media-list credit-history-list" id="creditHistoryList"></ul>
  </div>
      ...
      <div class="aui-media-list-item-inner">
        <div class="aui-list-item-inner">
          <div class="aui-list-item-text">
            <div class="aui-list-item-title title">{{= it[i]['operate_time']}}</div>
          </div>
          <div class="aui-list-item-text">
            <div class="aui-list-item-title title name"><?php echo $lang['label_previous_credit'].$lang['label_colon'];?> {{= it[i]['before_credit']}}</div>
          </div>
          <div class="aui-list-item-text">
            <div class="aui-list-item-title title name"><?php echo $lang['label_current_credit'].$lang['label_colon'];?> <em class="color66165d">{{= it[i]['current_credit']}}</em></div>
          </div>
        </div>
      </div>
    </li>
...
function getPageData(){
  $.ajax({
    type: 'POST',
    url: '<?php echo WAP_SITE_URL;?>/index.php?act=credit&op=getCreditHistoryData',
    ...
    success: function(data){
      if(data.STS){
        ...
        if(list == null){
          ...
          if(page_num == 1){
            $('#creditHistoryList').html(interText(list));
          }else{
            $('#creditHistoryList').append(interText(list));
          }
          if(total_pages <= 1){
            loadDataDone('<?php echo $lang['label_no_data'];?>');
          }else{
