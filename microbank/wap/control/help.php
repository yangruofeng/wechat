...
class helpControl
{
    ...
    public function helpCategoryOp()
    {
        Tpl::output('html_title', L('label_help_category'));
        Tpl::output('header_title', L('label_help_category'));
        $rt = $this->getHelpCategoryOp();
        $list = $rt->DATA;
        Tpl::output('list', $list);
        Tpl::showPage('help.category');
    }

    public function getHelpCategoryOp()
    {
        $data = $_POST;
        $url = ENTRY_API_SITE_URL . '/help.category.php';
