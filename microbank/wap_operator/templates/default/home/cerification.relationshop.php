...
$('#submit').on('click', function(){
  ...
  $.ajax({
    type: 'POST',
    url: '<?php echo WAP_OPERATOR_SITE_URL;?>/index.php?act=home&op=ajaxAddRelationship',
    data: {id: '<?php echo $_GET['id'];?>',relation_type: relation_type, guarantee_member_account: guarantee_member_account, country_code: country_code, phone: phone},
    dataType: 'json',
    success: function(data){
      ...
      if(data.STS){
        ...
        var times = setInterval(function(){
          ...
          if(count <= 1){
            clearInterval(times);
            window.location.href = "<?php echo getUrl('home', 'certTypeList', array('type'=>certificationTypeEnum::GUARANTEE_RELATIONSHIP,'back'=>'certList','id'=>$_GET['id']), false, WAP_OPERATOR_SITE_URL)?>";
          }
        },1000);
      }else{
        verifyFail(data.MSG);
