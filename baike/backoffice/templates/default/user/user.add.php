...
<div class="page">
    ...
    <div class="container" style="width: 600px;">
        <form class="form-horizontal" method="post">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <?php foreach ($output['user_position'] as $key => $val) { ?>
                        <label class="col-sm-5 radio-inline"><input type="radio" name="user_position" value="<?php echo $key?>"><?php echo ucwords(strtolower($val))?></label>
                    <?php }?>
                    <div class="error_msg"></div>
                </div>
            </div>
            <div class="form-group role-select" style="display: none">
                <label class="col-sm-3 control-label"><?php echo 'Role'?></label>
                <div class="col-sm-9">
                    <?php foreach($output['role_list'] as $val){?>
                        <label class="col-sm-4 checkbox-inline"><input type="checkbox" name="role_select[]" allow-back-office="<?php echo implode(',',$val['allow_back_office']['allow_auth'])?>" allow-counter="<?php echo implode(',',$val['allow_counter']['allow_auth'])?>" value="<?php echo $val['uid']?>"><?php echo $val['role_name']?></label>
                    <?php }?>
                </div>
            </div>
            <div class="form-group back_office_auth" style="display: none">
                            ...
                            <ul class="list-group">
                                <?php foreach($v_1 as $v_2){?>
                                    <li class="list-group-item col-sm-6">
                                        <label class="col-sm-4 checkbox-inline" title="<?php echo L('auth_'.strtolower($v_2))?>"><input type="checkbox" name="auth_select[]" value="<?php echo $v_2?>"><?php echo L('auth_'.strtolower($v_2))?></label>
                                    </li>
                                <?php }?>
                            </ul>
                        </div>
            ...
            <div class="form-group counter_auth" style="display: none">
                    ...
                    <?php foreach($output['auth_group_counter'] as $k_1=>$v_1){?>
                        <div style="font-size: 16px;margin-bottom: 5px" class="counter_auth_group clearfix">
                            ...
                            <ul class="list-group">
                                <?php foreach($v_1 as $v_2){?>
                                    <li class="list-group-item col-sm-6">
                                        <label class="col-sm-4 checkbox-inline" title="<?php echo L('auth_counter_'.strtolower($v_2))?>"><input type="checkbox" name="auth_select_counter[]" value="<?php echo $v_2?>"><?php echo L('auth_counter_'.strtolower($v_2))?></label>
                                    </li>
                                <?php }?>
                            </ul>
                        </div>
