            ...
            <?php foreach ($data['data'] as $key => $row) { ?>
                <tr>
                    ...
                    <td>
                        <div class="cert-info">
                            <p>
                                <label class="lab-name">Name :</label><?php echo $row['login_code'] ?>
                            </p>
                                ...
                                <?php if ($row['verify_state'] == 0) { ?>
                                    <div class="custom-btn-group">
                                        <a class="custom-btn custom-btn-secondary"
                                           href="<?php echo getUrl('operator', 'getTaskOfCertification', array('uid' => $row['uid']), false, BACK_OFFICE_SITE_URL) ?>">
                                ...
                                <?php } elseif ($row['verify_state'] == certStateEnum::LOCK && $data['cur_uid'] == $row['auditor_id']) { ?>
                                    <div class="custom-btn-group">
                                        <a title="Operator：Owner" class="custom-btn custom-btn-secondary"
                                           href="<?php echo getUrl('operator', 'showCertificationDetail', array('uid' => $row['uid']), false, BACK_OFFICE_SITE_URL) ?>">
