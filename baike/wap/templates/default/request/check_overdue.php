...
<div class="wrap request-wrap request-index-wrap">
  ...
  <div class="aui-tab" id="tab">
      <div class="aui-tab-item tab-1 aui-active" type="1">Checking</div>
      <div class="aui-tab-item tab-2" type="2">Done</div>
  </div>
...
<script type="text/html" id="tpl_request_item">
  {{ if( it && it.length > 0 ){ }}
    {{ for(var i = 0; i< it.length; i++) { }}
      <li class="aui-list-item aui-list-item-middle" onclick="requestDetail({{=it[i]['task_id']}});">
        <div class="aui-media-list-item-inner">
          <div class="aui-list-item-inner aui-list-item-arrow">
            <div class="aui-list-item-text">
              <div class="aui-list-item-title name">{{=it[i]['login_code']}}</div>
              <div class="aui-list-item-right amount">
                  {{=it[i]['currency']}} {{=it[i]['total']}}
              </div>
            </div>
            <div class="aui-list-item-text">
              <div class="aui-list-item-title"></div>
              <div class="aui-list-item-right time">
                  {{=it[i]['receivable_date']}}
              </div>
            </div>
            <div class="aui-list-item-text">
              <div class="aui-list-item-title phone"><img src="<?php echo WAP_OPERATOR_SITE_URL;?>/resource/image/verify_phone.png" />{{=it[i]['phone_id']}}</div>
              <div class="aui-list-item-right state">
                  {{=it[i]['contract_sn']}} {{=it[i]['scheme_name']}}
              </div>
            </div>
          </div>
        </div>
...
<script type="text/javascript">
  ...
  function requestDetail(uid){
    window.location.href = '<?php echo WAP_OPERATOR_SITE_URL?>/index.php?act=request&op=dealOverdueContract&uid='+uid;
  }
  ...
  function getPageData(){
    $.ajax({
      type: 'get',
      url: '<?php echo WAP_OPERATOR_SITE_URL;?>/index.php?act=request&op=getOverdueData',
