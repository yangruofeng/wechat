...
class page{
	...
	public function __construct(){
		...
		$this->pre_home = "First"; // $lang['first_page'];
		$this->pre_last = "Last"; // $lang['last_page'];
		$this->pre_page = "Previous"; // $lang['pre_page'];
		$this->next_page = "Next"; // $lang['next_page'];
		/**
		 * 设置当前页码
		 */
		...
		switch ($this->style) {
            ...
            case '6':
                ...
                if ($this->getNowPage() == 1){
                    ...
                    $html_page .= '<li><a class="demo" href="javascript:get_goods_list(1)">'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</a></li>';
                    $html_page .= '<li><a class="demo" href="javascript:get_goods_list('.($this->getNowPage()-1) .')">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a></li>';
                }
                $html_page .= $this->getNowBar('special');
                if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
                    $html_page .= '<li>'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</li>';
                    $html_page .= '<li>'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</li>';
                }else {
                    $html_page .= '<li><a class="demo" href="javascript:get_goods_list('.($this->getNowPage()+1) .')">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a></li>';
                    $html_page .= '<li><a class="demo" href="javascript:get_goods_list('.$this->getTotalPage() .')">'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</a></li>';
                }
                $html_page .= '</ul>';
                break;
			default:
	 ...
	 * @return string 字符串类型的返回结果
	 */
	private function getNowBar($mark=''){
		/**
		 * 显示效果
        ...
        if($mark){
            $begin=($this->getNowPage()-4>0)?$this->getNowPage()-3:1;
        }
		/**
		 * 计算结束页码
		 */
		...
		for ($i=$begin;$i<=$end;$i++){
            if(!$mark){
                ...
                $result .= $this->setPageHtmlSpec($i,$i);
            }
		}
	 ...
	 * @return string 字符串类型的返回结果
	 */
	private function setPageHtmlSpec($page_name,$page){
		/**
		 * 判断是否是当前页
		 */
		if ($this->getNowPage() == $page){
            ...
            $result = $this->left_html."<a class='demo' href='javascript:get_goods_list(". $page .")'>".$this->left_inside_a_html.$page_name.$this->right_inside_a_html."</a>".$this->right_html;
		}
		return $result;
	}
...
/* /shop/article_cate-1.html
 * after apache url rewrite:
 * $_SERVER['QUERY_STRING'] : ''
 * $_SERVER['REQUEST_URI'] /shop/article_cate-1.html
 * after nginx url rewrite:
 * $_SERVER['QUERY_STRING'] : 'act=article&op=article&ac_id=1'
 * $_SERVER['REQUEST_URI'] /shop/article_cate-1.html
 */
		/**
		 * 不存在QUERY_STRING时
		 */
		if(empty($_SERVER['QUERY_STRING'])){//when nginx rewrite url, the query string is not empty
			...
			if(stristr($_SERVER['QUERY_STRING'],$this->page_name.'=')){
				...
				}else if(strpos($this->page_url,'?') === false){
					$this->page_url .= '?'.$this->page_name."=";
				}else{
					$this->page_url .= '&'.$this->page_name."=";
				}
    ...
    private function setShopPseudoStaticPageUrl($page){
        ...
        return urlDesktop($act, $op, $param);
    }
}
